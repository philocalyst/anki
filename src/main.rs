...
use flash::{
	self,
	change_resolver::{resolve_changes, IdentifiedNote},
	change_router::determine_changes,
	deck_locator::DeckLocator,
	model_loader,
	print_note_debug,
	types::{deck::Deck, note::ONote},
};
...
fn main() -> Result<()> {
	...
	// Find and scan deck
	let deck_path =
		DeckLocator::find_deck_directory().wrap_err("Failed to find deck directory")?;
	info!("Found deck at: {:?}", deck_path);

	let (model_paths, card_paths) =
		DeckLocator::scan_deck_contents(&deck_path).wrap_err("Failed to scan deck contents")?;
	...
	let card_content =
		fs::read_to_string(first_card_path).wrap_err("Failed to read card file")?;

	let cards =
		deck.parse_cards(&card_content).wrap_err("Failed to parse cards")?;
	...
	// the token of trade
	let history = deck
		.get_file_history("index.flash")
		.wrap_err("Failed to get file history")?;
	let mut point = 0;
	...
	while point < history.len() {
		...
		let content = deck
			.read_file_content(&active_entry)
