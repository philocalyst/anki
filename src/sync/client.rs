...
impl FlashClient {
	...
	pub async fn save_deck_config(&self, config: &serde_json::Map<String, Value>) -> Result<()> {
		self
			.0
			.misc()
			.multi(&[MultiAction::with_params("saveDeckConfig", serde_json::json!({"config": config}))])
			.await
...
/// Find a unique name by appending a suffix counter when the base name is
/// taken. e.g. if "Basic" is taken, tries "Basic 2", "Basic 3", etc.
